...
echo '8. '
cat data-hold/legislators-current.json | \
  jq '.[] .terms[0] .start' --raw-output | \
    cut -d '-' -f 1 | sort -n | head -1
echo '9. '
cat data-hold/legislators-current.json | \
  jq --raw-output '.[] .terms[-1:][0].party' | grep -c 'Republican'
echo '10. '
cat data-hold/legislators-current.json | \
  jq --raw-output '.[]  .terms[-1:][0] .party' | grep -c 'Independent'
echo '11. '
cat data-hold/legislators-current.json | \
...
cat data-hold/legislators-social-media.json | \
  jq '.[] .social .twitter' | grep -v 'null' | wc -l
echo '13. '
cat data-hold/legislators-social-media.json | \
  jq '.[] .social .facebook' | grep -v 'null' | wc -l
echo '14. '
cat data-hold/legislators-social-media.json | \
...
echo '15. ' 
cat data-hold/legislators-social-media.json | \
  jq --raw-output '.[]  | [.id .bioguide, .social .twitter] | @csv' | grep  '\,$'
echo '16. '
cat data-hold/congress-twitter-profiles.json | \
  jq '.[] .verified' | grep 'true' | wc -l
echo '17. '
cat data-hold/congress-twitter-profiles.json | \
  jq '.[] .followers_count' | sort -nr | head -1
echo '18. '
cat data-hold/congress-twitter-profiles.json | \
